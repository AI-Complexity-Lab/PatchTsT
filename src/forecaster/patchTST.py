...
class PatchTST(nn.Module):
    ...
    """
    def __init__(self, out_layer_dim, hidden_dim, device, weeks_ahead:int, c_in:int, target_dim:int, patch_len:int, stride:int, num_patch:int, 
        ...
        self.out_layer = [
            ...
            nn.ReLU(),
            nn.Linear(in_features=out_layer_width // 2, out_features=1),
        ]
    ...
    def forward(self, z, output_emb=False):                             
        """
        z: tensor [bs x num_patch x n_vars x patch_len]
        """   
        num_patch = 1
        bs = z.size(0) // num_patch
        n_vars = z.size(2)
        patch_len = z.size(1)
        z = z.view(bs, num_patch, n_vars, patch_len)
        if bs < 32:
            delta = 32 - bs
            padding_value = z[-1, :, :, :]
            padding = padding_value.repeat(delta, 1, 1, 1)
        ...
        #    [bs x target_dim] for classification
        #    [bs x num_patch x n_vars x patch_len] for pretrain
        position = torch.arange(1, self.weeks_ahead + 1).repeat(
            z.shape[0], 1).unsqueeze(2)
        # create a position tensor of shape (batch_size, seq_len, 1) with values 0 to 1
        position = ((position - position.min()) /
                    (position.max() - position.min())).to(self.device)
        emb = self.decoder(position, z)
        # emb: torch.Size([32, 4, 64])
        out = self.out_layer(emb)
        # out: torch.Size([32, 4, 1])
        if output_emb:
            return out, emb
        return out
