...
def aggragate_pred(dir_path, weeks_ahead=4):
    # create dicts
    preds = {}
    ...
    return preds, regions_in_exp


def combine_predictions_all(preds, regions_in_exp):
    pred_weeks = None
...
def viz_helper(results, viz_region, CLs, gt_length=10, savefig=False, results_path='', CL2display=None):
    ...
    data_file_week = (Week.fromstring(test_week)).cdcformat()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--viz_region', default='all')
    ...
    viz = args.viz
    viz_region = args.viz_region
    seed = int(args.seed)

    fname = 'results.pickle'
    if seed != -1:
        fname = f'results_{seed}.pickle'
    ...
    results = pickle_load(dir_path / fname)
    ...
    # save test result
    if save_test:
        output_path = res_dir / f'test_results.csv'
        write_test_results(test_results, output_path)
    else:
        # process predictions
        # get preds and regions using aggragate_preds first if there are multiple prediction file (in the older version)
        preds, regions, pred_weeks = combine_predictions_all(pred_results, regions)

        if viz:
            if viz_region == 'all':
                for region in regions:
                    viz_helper(viz_region=region, results=results, CLs=CLs, savefig=True, results_path=results_path, CL2display=[0.1, 0.3, 0.5, 0.7, 0.9])
            else:
                viz_helper(viz_region=viz_region, results=results, CLs=CLs)
            exit(0)
        ...
        # text results
        write_exp_results(preds, regions, res_dir/f'{exp_id}.csv', weeks_ahead=weeks_ahead)

        if plot:
            # plots
            for alpha in alpha_list:
                # regions = get_all_regions(dir_path, combined_file)
                for region in tqdm.tqdm(regions):
                    for w in range(weeks_ahead):
                        fig_dir_path = results_path / f'pred_plots_alpha_{alpha}'
                        if not fig_dir_path.exists():
                            fig_dir_path.mkdir()
                        fig_path = fig_dir_path / f'{region}_{w}.png'
                        region_preds = combine_predictions(dir_path, w, alpha, region, return_dict=True, all_preds=preds, pred_weeks=pred_weeks)
                        plot_predictions(region_preds, w, figpath=fig_path, savefig=True)
