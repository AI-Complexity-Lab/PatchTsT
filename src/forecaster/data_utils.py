...
def epiweek_sub(week1, week2, max_attempts=200):
    # assumes week1 is no less than week2
...
def get_state_train_data(params, region, smooth=False):
    ...
    # convert to epiweeks
    start_week = convert_to_epiweek(params['data_params']['start_week'])
    last_train_week = convert_to_epiweek(params['last_train_week'])
...
def get_state_test_data_xy(params, region, pred_week, x_length, smooth=False, remove_weeks_after_test=True):
    ...
    # y
    test_week = convert_to_epiweek(params['test_week'])
    ...
    # x
    df = load_df(params, region, start_week, pred_week, smooth).tail(x_length)
...
def prepare_ds(xs, xs_masks, ys, ys_masks, regions, metas, test=False, return_ds=True):
    ...
    if not test:
        ...
        ys, ys_masks = np.zeros_like(regions), np.zeros_like(regions)

    if return_ds:
        dataset = SeqData(regions, metas, xs, xs_masks, ys, ys_masks)
        return dataset
...
class SeqData(torch.utils.data.Dataset):
    ...
    def __init__(self, region, metas, X, mask_X, y, mask_y):
        self.region = region
        self.metas = metas
        self.X = X
        self.mask_X = mask_X
